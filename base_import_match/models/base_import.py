...
class BaseImportMatch(models.Model):
    ...
    def _compute_name(self):
        ...
        for one in self:
            field_display_names = [name for name in one.field_ids.mapped("display_name") if name]
            field_display_names_str = " + ".join(field_display_names)

            one.name = "{}{}".format(
                one.model_id.display_name,
                ": " + field_display_names_str if field_display_names_str else ''
            )
            # print(one.field_ids.mapped("display_name"))
            # one.name = "{}: {}".format(
            #     one.model_id.display_name,
